...
#    load_all_images
    ...
    docker run --rm \
      ...
      -v `pwd`/clean_components.sh:/pangolin/clean_components.sh \
      ${RELEASE_IMAGE} \
      sh -c "bash /pangolin/clean_components.sh && python3 amctl.py init all \
      && until /pangolin/kubectl get pods | grep am-minion | grep Running; do echo 'waiting for master to be ready'; sleep 5; done; \
