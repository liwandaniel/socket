...
- [产品自动化打包发布](#%E4%BA%A7%E5%93%81%E8%87%AA%E5%8A%A8%E5%8C%96%E6%89%93%E5%8C%85%E5%8F%91%E5%B8%83)
  ...
  - [开始自动化打包](#%E5%BC%80%E5%A7%8B%E8%87%AA%E5%8A%A8%E5%8C%96%E6%89%93%E5%8C%85)
...
- `oss_path`: 上传 oss 路径，例如 `compass-v2.7.3-rc/01` 
...
- `release`: 选择是否进行构建镜像的操作，默认为 true
...
- `release_registry`: 打包发布的镜像仓库域名，即上面部署用于发布的仓库
...
![image](https://user-images.githubusercontent.com/25719123/48329930-c2521c00-e685-11e8-9c72-c1ccb155ea0d.png)
...
1. input: 可选参数为 `sync, judge, package, upload`, 传递不同的参数以执行不同的逻辑
    ...
    - package: 打包
    - upload: 上传oss
2. version: 发布版本，例如 `v2.7.3-rc01`
3. cargo_dir: cargo 部署的路径，即该目录下有保存镜像文件的 `./common/cargo-registry` 目录，例如 `/var/lib/kubelet/compass`
4. sync_dir: [镜像列表](../images-lists)和[镜像同步脚本](../hack/sync_images_scripts/sync.sh)保存的路径，例如 `/root/sync-scripts`
5. oss_path: oss 上传的路径，例如 `compass-v2.7.3-rc/01`，最终上传路径为 `oss://infra-release/platform/compass-v2.7.3-rc/01/compass-component-v2.7.3-rc01.tar.gz`
...
$ bash package.sh sync v2.7.3-rc01 /var/lib/kubelet/compass /root/sync-scripts
```
...
```bash
$ bash package.sh judge v2.7.3-rc01 /var/lib/kubelet/compass /root/sync-scripts
...
```bash
bash package.sh package v2.7.3-rc01 /var/lib/kubelet/compass
```
...
```bash
bash /root/package.sh upload v2.7.3-rc01 /var/lib/kubelet/compass /root/sync-scripts compass-v2.7.3-rc/01
```
